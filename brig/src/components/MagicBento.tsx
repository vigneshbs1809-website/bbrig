...
const DEFAULT_GLOW_COLOR = '247, 152, 130'; // Coral accent color
    ...
    <>
      ...
      <BentoCardGrid gridRef={gridRef}>
        {cards.map((card, index) => {
          const baseClassName = `magic-bento-card ${textAutoHide ? 'magic-bento-card--text-autohide' : ''} ${enableBorderGlow ? 'magic-bento-card--border-glow' : ''} light-emit-border`;
          
          const handleCardClick = card.href 
...
export const BentoButton: React.FC<BentoButtonProps> = ({
  ...
  const animateParticles = useCallback(() => {
    if (!enableStars || !buttonRef.current || !isHoveredRef.current) return;
  ...
  }, [animateParticles, clearAllParticles, enableStars, enableTilt, enableMagnetism, clickEffect, glowColor, onClick]);
