...
class Media {
  ...
  constructor({
    ...
    const isVideo = this.image.toLowerCase().endsWith('.mp4') || 
                    this.image.toLowerCase().endsWith('.webm') || 
                    this.image.toLowerCase().endsWith('.ogg');
  ...
  update(scroll: any, direction: string) {
    ...
    if (this.isVideo && this.video && this.video.readyState >= this.video.HAVE_CURRENT_DATA) {
